    ...
    <table class="min-w-[800px] w-full relative">
      <thead class="border-b sticky top-0 bg-white z-[1]">
        <tr>
          ...
          <th class="px-4 text-sm font-semibold tracking-wide text-left">IP</th>
          <th class="px-4 text-sm font-semibold tracking-wide text-left">
            Created At
          </th>
          <th class="px-4 text-sm font-semibold tracking-wide text-left">
      ...
      <tbody class="">
        ...
        >
          ...
          <td class="frozen-column px-4 text-sm">{{ activity.id }}</td>
          ...
          <td class="td-created-at px-4 text-sm">{{ activity.subject }}</td>
          <td class="td-updated-at px-4 text-sm">{{ activity.url }}</td>
          <td class="td-updated-at px-4 text-sm">{{ activity.method }}</td>
          <td class="td-updated-at px-4 text-sm">{{ activity.ip }}</td>
          <td class="td-updated-at px-4 text-sm">{{ activity.created_at }}</td>
          <td class="td-updated-at px-4 text-sm">{{ activity.updated_at }}</td>
          <td class="px-4 text-sm w-full relative">
            <div>
              <KebabMenu
                @menuOpen="activitylogsStore.openMenu(activity.id)"
            ...
            <PopupMenu
              @viewClicked="
                {
                  activitylogsStore.showActivityData(activity.id);
                  activitylogsStore.toggleMenu(activity.id);
                }
              "
              v-if="activity.isMenuActive"
              :action1="true"
...
export default {
  ...
  setup() {
    ...
    onClickOutside(target, (event) => activitylogsStore.closeMenus());

    return { activitylogsStore, navbarStore, target };
  },
  methods: {
    // openMenu(ID) {
