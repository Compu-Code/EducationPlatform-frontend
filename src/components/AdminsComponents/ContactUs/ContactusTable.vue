    ...
    <table class="min-w-[800px] w-full relative">
      ...
      <tbody class="">
        ...
        >
          ...
          <td class="frozen-column px-4 text-sm">{{ contactUs.id }}</td>
          ...
          <td class="td-created-at px-4 text-sm">{{ contactUs.email }}</td>
          <td class="td-updated-at px-4 text-sm">{{ contactUs.subjact }}</td>
          <td class="td-updated-at px-4 text-sm">
            {{ contactUs.description }}
          </td>
          <td class="td-updated-at px-4 text-sm">{{ contactUs.created_at }}</td>
          <td class="td-updated-at px-4 text-sm">{{ contactUs.updated_at }}</td>
          <td class="px-4 text-sm w-full relative">
            <div>
              <KebabMenu
                @menuOpen="contactUsStore.openMenu(contactUs.id)"
            ...
            <PopupMenu
              @viewClicked="
                {
                  contactUsStore.showContactUsData(contactUs.id);
                  contactUsStore.toggleMenu(contactUs.id);
                }
              "
              @deleteClicked="
                contactUsStore.deleteContactUs(contactUs.id);
                contactUsStore.toggleMenu(contactUs.id);
              "
...
export default {
  ...
  setup() {
    ...
    onClickOutside(target, (event) => contactUsStore.closeMenus());

    return { contactUsStore, navbarStore, target };
  },
  methods: {
    // openMenu(ID) {
