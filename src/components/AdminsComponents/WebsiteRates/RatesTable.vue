    ...
    <table class="min-w-[800px] w-full relative">
      ...
      <tbody class="">
        ...
        >
          ...
          <td class="frozen-column px-4 text-sm">{{ rate.id }}</td>
          ...
          <td class="td-created-at px-4 text-sm">{{ rate.created_at }}</td>
          <td class="td-updated-at px-4 text-sm">{{ rate.updated_at }}</td>
          <td class="px-4 text-sm w-full relative">
            <div>
              <KebabMenu
                @menuOpen="ratesStore.openMenu(rate.id)"
            ...
            <PopupMenu
              @viewClicked="
                {
                  ratesStore.showRateData(rate.id);
                  ratesStore.toggleMenu(rate.id);
                }
              "
              @deleteClicked="
                ratesStore.deleteRate(rate.id);
                ratesStore.toggleMenu(rate.id);
              "
              v-if="rate.isMenuActive"
              :action1="true"
              :action4="true"
...
export default {
  ...
  setup() {
    ...
    onClickOutside(target, (event) => ratesStore.closeMenus());

    return { ratesStore, navbarStore, target };
  },
  methods: {
    // openMenu(ID) {
