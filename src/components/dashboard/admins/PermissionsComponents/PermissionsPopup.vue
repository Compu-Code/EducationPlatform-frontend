    ...
    <div class="Container p-4 w-full">
      ...
      <form @submit.prevent="" class="flex flex-col px-3 pt-3">
        <input
          ...
          @keyup="getPermissionName($event.target.value)"
          @blur="getPermissionName($event.target.value)"
          v-model="permissionName"
        />
        <p>Select Roles For Permission :</p>
        <div
        ...
        <div class="flex">
          ...
          >
            <span v-if="!permissionsStore.createLoading">Create</span>
            <div class="spinner" v-if="permissionsStore.createLoading"></div>
          </normal-filled>
          <normal-filled
          ...
          >
            <span v-if="!permissionsStore.editLoading">Confirm</span>
            <div class="spinner" v-if="permissionsStore.editLoading"></div>
          </normal-filled>
...
export default {
  ...
  watch: {
    ...
    editPermissionRoles(newValue, oldValue) {
  ...
  methods: {
    ...
    async createPermission() {
      if (this.permissionName) {
        await this.permissionsStore.createPermission({
          name: this.permissionName,
          roles: this.selectedRoles,
        });
        console.log(this.permissionsStore.createdSuccessfully);
        if (this.permissionsStore.createdSuccessfully) {
          this.$emit("close");
        }
      }
    },
    ...
    async updatePermission() {
      ...
      if (this.selectedRoles) {
        this.permissionName = document.getElementsByName("name")[0].value;
      }
      if (this.permissionName && this.selectedRoles) {
        await this.permissionsStore.updatePermission(
          {
        ...
        if (this.permissionsStore.updatedSuccessfully) {
          this.$emit("close");
        }
      }
    },
  ...
  mounted() {
    ...
    if (this.editPermissionRoles) {
      if (this.editPermissionRoles.length > 0) {
        this.selectedRoles = this.editPermissionRoles;
      }
    }
    if (this.editPermissionName) {
      if (this.editPermissionName.length > 0) {
        this.permissionName = this.editPermissionName;
      }
    }
  },
  unmounted() {
