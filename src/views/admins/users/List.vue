<template>
  <main
    class="users mt-6 w-full duration-300 mb-10"
    :class="[
  ...
  >
    ...
    <div
      class="table-sh"
      :class="[navbarStore.darkMode ? ' bg-[#2E3239]' : 'bg-white']"
    >
      <UsersTable />
    </div>
  </main>
...
const UsersTable = defineAsyncComponent(() =>
  import("@/components/dashboard/admins/UsersComponents/UsersTable.vue")
);
...
export default {
  ...
  setup() {
    ...
    return { navbarStore, SidebarStore, usersStore };
  },
  data() {
    return {};
  },
  mounted() {
    this.usersStore.getUsersData();
  },
};
</script>
